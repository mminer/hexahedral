...
/***/ function(module, exports, __webpack_require__) {
	...
	var maxMoves = Infinity;
	var moveCount = 0;
	var player = {};
	var tiles = {};

	var keyHandlers = (_keyHandlers = {}, _defineProperty(_keyHandlers, keyCodes.LEFT, function () {
	  move(0, -1);
	}), _defineProperty(_keyHandlers, keyCodes.UP, function () {
	  move(-1, 0);
	}), _defineProperty(_keyHandlers, keyCodes.RIGHT, function () {
	  move(0, 1);
	}), _defineProperty(_keyHandlers, keyCodes.DOWN, function () {
	  move(1, 0);
	}), _defineProperty(_keyHandlers, keyCodes.R, function () {
	  reset();
	}), _keyHandlers);
	...
	function move(rowDelta, columnDelta) {
	  var newRow = player.row + rowDelta;
	  var newColumn = player.column + columnDelta;
	  moveTo(newRow, newColumn);
	}

	// Moves the player to a specific location.
	function moveTo(row, column) {
	  if (!(0, _levelManager.canMoveTo)(tiles, row, column)) {
	    return;
	  }

	  var moveDistance = (0, _util.findDistance)(player.row, player.column, row, column);
	  var invalidMoveDistance = moveDistance !== 1;
	  ...
	  moveCount += 1;
	  (0, _util.playSoundEffect)(moveAudio);
	  update();
	}
	...
	function update() {
	  ...
	  var columnCount = tiles[0].length;

	  _d2.default.select('body').attr('class', gameStatus.toLowerCase());

	  _d2.default.select('main').style({
	...
	function updateLevel() {
	  var selection = _d2.default.select('main').selectAll('.cell').data(function () {
	    return tiles.reduce(function (cellArray, rowTiles, row) {
	      var cells = rowTiles.map(function (tile, column) {
	        return { tile: tile, row: row, column: column };
	      });
	  ...
	  selection.classed({
	    ...
	    return moveTo(d.row, d.column);
	  }).style({
	    left: function left(d) {
	      return d.column * SQUARE_SIZE + 'rem';
	    },
	    top: function top(d) {
	      return d.row * SQUARE_SIZE + 'rem';
	    }
	  });
	...
	function updateLevelNavigator() {
	  var currentLevelNumber = (0, _levelManager.levelNumberFromHash)();

	  var data = _d2.default.range(_levels2.default.length).map(function (levelNumber) {
	    return {
	  ...
	  var selection = _d2.default.select('nav').selectAll('.level-button').data(data);

	  // Enter
	  selection.enter().append('button').attr({
	    'class': 'level-button',
	    'title': function title(d) {
	      return 'Level ' + d.levelNumber;
	    },
	    'type': 'button'
	  }).on('click', function (d) {
	...
	function updateMoveCounter() {
	  ...
	  var selection = _d2.default.select('header').selectAll('.counter').data(data);
	  ...
	  // Exit
	  selection.exit().transition().duration(200).style('width', '0px').remove();
	}
	...
	function loadLevel(levelNumber) {
	  var levelExists = levelNumber in _levels2.default;
	...
	// Loads the level.
	function reset() {
	  var levelNumber = (0, _levelManager.levelNumberFromHash)();
	  ...
	  tiles = level.tiles.map(function (rowTiles) {
